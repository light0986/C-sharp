            ...
            if (input != null && input.Rows.Count > 0)
            {
                foreach (DataRow row in input.Rows)
                {
                    ...
                    foreach (DataColumn col in input.Columns)
                    {
                        ...
                        {
                            jsonBuilder.Append("\"" + col.ColumnName + "\":\"" + row[col.ColumnName].ToString().Trim() + "\",");
                        }
                        else if (type == "Boolean")
                        {
                            bool? bo = row[col.ColumnName] as bool?;
                            if (bo == true) { jsonBuilder.Append("\"" + col.ColumnName + "\":1,"); }
                            else if (bo == false) { jsonBuilder.Append("\"" + col.ColumnName + "\":0,"); }
                            else { jsonBuilder.Append("\"" + col.ColumnName + "\":null,"); }
                        }
                        else
                        {
